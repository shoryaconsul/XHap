# This version of XHap takes a sparse input (used for larger read=SNP matrices)
# and runs XHap on chunks before combining the results. This is done to reduce
# memory usage.
...
from read_embeddings import save_ckp, load_ckp, MyFilter, \
    ReadAE, SparseSNVMatrixDataset, chunk_data, learn_embed
...
def det_memhap(SNVdataset: SparseSNVMatrixDataset,
...
def train_xhap(SNVdata: SparseSNVMatrixDataset, gpu: int=-1, d_model: int = 128, num_hap: int = 2,
               num_epoch: int = 3000, verbose: bool=False
               )  -> NDArray[(Any, Any), int]:
    """
    ...
    --------
    NDArray[(Any, Any), int]:
        Haplotype matrix with lowest MEC
    """
    ...
    # datapath = 'generate_data/' + outhead + '_SNV_matrix.txt'  
    # pos_file = 'generate_data/' + outhead + '_SNV_pos.txt'

    
    # SNV_matrix_list = chunk_data(datapath, chunk_size=250, overlap_frac=0.4)
    ...
    total_params = (sum(p.numel() for p in embedAE.parameters())
                        + sum(p.numel() for p in corr_xformer.parameters()))
    ...
    xformer_savefile = None  # Do not save intermediate models
    ...
    # print("SNV matrix: ", SNV_matrix.shape)
    # print("Haplotype matrix: ", hap_matrix.shape)
    ...
    return hap_matrix_best, mec_min

def readW(SNVdataset: SparseSNVMatrixDataset, 
          ae: ReadAE, xformer: CorrTransformer,
          device: torch.cuda.device=torch.device("cpu")
         ):
